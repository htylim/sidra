...
type ExtensionToBridge =
  | { type: "session.start"; version: 3; clientSessionId: string; providerId: "codex" }
  | { type: "session.send"; version: 3; clientSessionId: string; prompt: string; promptEffort?: PromptEffort; pageContext?: PageContext }
...
type BridgeToExtension =
  | { type: "session.started"; version: 3; clientSessionId: string; bridgeSessionId: string }
  | { type: "agent.event"; version: 3; clientSessionId: string; event: AgentEvent }
  | { type: "permission.request"; version: 3; clientSessionId: string; request: PermissionRequest }
  | { type: "session.error"; version: 3; clientSessionId: string; message: string; code?: string }
  | { type: "bridge.ready"; version: 3 }
  | { type: "bridge.error"; version: 3; message: string; code?: string };
```
