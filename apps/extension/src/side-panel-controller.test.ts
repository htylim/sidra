...
describe("SidePanelController", () => {
  ...
  it("queues the first prompt until the bridge session starts", () => {
    ...
    expect(ports[0].postedMessages).toEqual([
      { type: "session.start", version: 4, clientSessionId: "client-1", providerId: "codex" },
      { type: "session.send", version: 4, clientSessionId: "client-1", prompt: "hello", promptEffort: "medium" }
    ]);
  });
  ...
  it("rejects a second prompt while the current turn is queued", () => {
    ...
    expect(ports[0].postedMessages).toEqual([
      { type: "session.start", version: 4, clientSessionId: "client-1", providerId: "codex" },
      { type: "session.send", version: 4, clientSessionId: "client-1", prompt: "first", promptEffort: "medium" }
    ]);
  });
  ...
  it("starts a new provider session before sending after retrying a native bridge disconnect", () => {
    ...
    expect(ports[1].postedMessages).toEqual([
      { type: "session.start", version: 4, clientSessionId: "client-1", providerId: "codex" },
      { type: "session.send", version: 4, clientSessionId: "client-1", prompt: "second", promptEffort: "medium" }
    ]);
  });
...
describe("SidePanelController newChat", () => {
  ...
  it("uses the fresh session.started event after reset before sending later prompts", () => {
    ...
    expect(ports[0].postedMessages).toEqual([
      ...
      { type: "session.send", version: 4, clientSessionId: "client-1", prompt: "first", promptEffort: "medium" },
      { type: "session.reset", version: 4, clientSessionId: "client-1" },
      { type: "session.send", version: 4, clientSessionId: "client-1", prompt: "after reset", promptEffort: "medium" }
    ]);
  });
...
describe("SidePanelController transcript speech", () => {
  it("toggleSpeechForTranscriptEntry_posts_speech_synthesize_message", async () => {
    const { controller, ports } = createHarnessWithOptions({ createSpeechRequestId: () => "speech-1" });
    ports[0].emitMessage(bridgeReady());
    await waitForControllerSettings();

    expect(controller.toggleSpeechForTranscriptEntry("entry-1", "  Read this bubble.  ")).toBe(true);
    ...
    expect(controller.getSnapshot().speech).toMatchObject({ activeEntryId: "entry-1", status: "loading" });
  });

  it("newChat_stops_active_speech_request", () => {
    const { controller, ports } = createHarnessWithOptions({ createSpeechRequestId: () => "speech-1" });
    ports[0].emitMessage(bridgeReady());
    controller.toggleSpeechForTranscriptEntry("entry-1", "Read this bubble.");
  ...
  it("bridge_disconnect_stops_active_speech_request", () => {
    const { controller, ports } = createHarnessWithOptions({ createSpeechRequestId: () => "speech-1" });
    ports[0].emitMessage(bridgeReady());
    controller.toggleSpeechForTranscriptEntry("entry-1", "Read this bubble.");
  ...
  it("active_page_change_stops_active_speech_request", () => {
    const { controller, ports, activePage } = createHarnessWithOptions({ createSpeechRequestId: () => "speech-1" });
    ports[0].emitMessage(bridgeReady());
    controller.toggleSpeechForTranscriptEntry("entry-1", "Read this bubble.");
