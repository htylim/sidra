...
describe("native messaging connection cleanup", () => {
  ...
  it("native_disconnect_during_in_flight_turn_aborts_and_closes_provider_session", async () => {
    ...
    input.write(
      Buffer.concat([
        encodeNativeMessage({ type: "session.start", version: 2, clientSessionId: "page-1", providerId: "codex" }),
        encodeNativeMessage({ type: "session.send", version: 2, clientSessionId: "page-1", prompt: "Long prompt" })
      ])
    );
