...
describe("mock bridge chat path", () => {
  ...
  it("emits invalid_message bridge.error for unknown extension commands", async () => {
    ...
    await bridge.handleMessage({ type: "session.delete", version: 4, clientSessionId: "page-1" });
  ...
  it("emits invalid_message bridge.error for malformed session.start", async () => {
    ...
    await bridge.handleMessage({ type: "session.start", version: 4, clientSessionId: "page-1" });
  ...
  it("emits_invalid_message_bridge_error_for_non_json_serializable_direct_input", async () => {
    ...
    expect(emitted).toEqual([
      { type: "bridge.error", version: 4, message: "Message must be valid JSON", code: "invalid_message" },
      { type: "bridge.error", version: 4, message: "Message must be valid JSON", code: "invalid_message" },
      { type: "bridge.error", version: 4, message: "Message must be valid JSON", code: "invalid_message" }
    ]);
  });
  ...
  it("bridge_smoke_cleans_up_session_on_session_close", async () => {
    ...
    await bridge.handleMessage({ type: "session.close", version: 4, clientSessionId: "page-1" });
